...
#let diagram-right-triangle() = block(
  ...
  context [
    #place(dx: 40%, dy: 20pt, polygon(fill: blue.lighten(80%), stroke: 1.5pt + blue.darken(20%), (0pt, 90pt), (120pt, 90pt), (0pt, 0pt)))
    // Right angle indicator box
    #place(dx: 40%, dy: 95pt, rect(width: 10pt, height: 10pt, stroke: 1pt + blue.darken(20%)))
    // Labels
    #place(dx: 36%, dy: 60pt)[$a$]
    #place(dx: 55%, dy: 105pt)[$b$]
    #place(dx: 54%, dy: 45pt)[$c$]
  ]
)
...
#let diagram-algebraic-proof() = block(
  ...
  context [
    #place(dx: 42%, dy: 20pt)[
      ...
      // Labels for sides
      #place(dx: 10pt, dy: -15pt)[$a$]
      #place(dx: 60pt, dy: -15pt)[$b$]
      #place(dx: 45pt, dy: 45pt)[$c^2$]
    ]
  ]
)
...
#let diagram-coordinate-distance() = block(
  ...
  context [
    #place(dx: 35%, dy: 20pt)[
      // Axes
      #line(start: (0pt, 110pt), end: (150pt, 110pt), stroke: 1pt + gray.darken(40%)) // X axis
      #line(start: (20pt, 0pt), end: (20pt, 120pt), stroke: 1pt + gray.darken(40%))  // Y axis
      // Triangle representing distance
      #polygon(fill: blue.lighten(85%), stroke: 1.5pt + blue, (50pt, 80pt), (120pt, 80pt), (120pt, 30pt))
      // Points
      #place(dx: 30pt, dy: 85pt)[$P_1$]
      #place(dx: 125pt, dy: 20pt)[$P_2$]
      #place(dx: 80pt, dy: 40pt)[$d$]
      #place(dx: 80pt, dy: 85pt)[$Delta x$]
      #place(dx: 125pt, dy: 50pt)[$Delta y$]
    ]
  ]
)

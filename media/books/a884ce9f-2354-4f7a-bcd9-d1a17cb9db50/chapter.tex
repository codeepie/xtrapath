...
\begin{enumerate}
    \item \textbf{Drone Navigation Vectors:} A quadcopter flies from a launch pad with a velocity vector $\vec{v} = 8\hat{i} + 6\hat{j}$ meters per second. Calculate its total ground speed $\vert{}\vec{v}\vert{}$ and the heading angle $\theta$ relative to the positive $x$-axis.
    ...
    \begin{tikzpicture}[scale=0.5]
        ...
        % Vectors (rewritten statically to avoid calc syntax errors)

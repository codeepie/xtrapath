...
\definecolor{labblue}{HTML}{0056B3}

\begin{document}

\noindent
% --- EDITOR VISIBILITY CHECK ---
% Section divider for the dynamics module.
...
\begin{enumerate}
    ...
    \begin{tikzpicture}[scale=0.5]
        ...
        % Object position at 30 degrees
        \coordinate (O) at (0,0);
        \coordinate (P) at (4.33, 2.5);
        \draw[fill=black] (P) circle (0.2) node[above right] {Vehicle};
        % Vectors
        \draw[->, thick, blue] (O) -- (P) node[midway, above left] {$\vec{r}$};
        \draw[->, thick, red] (P) -- ($(P)!1.5!-90:(O)$) node[right] {$\vec{v}$};
        \draw[->, thick, orange] (P) -- ($(P)!2.5!(O)$) node[midway, below right] {$\vec{a}_c$};
    \end{tikzpicture}
    \end{center}
    \vspace{10pt}
    ...
    \begin{tikzpicture}[scale=0.7]
        ...
        \begin{scope}[shift={(4,2.31)}, rotate=30]
            \draw[thick, fill=gray!30] (-1,-0.5) rect (1,0.5);

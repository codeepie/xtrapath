                ...
                <div id="createChoiceModal" class="create-choice-overlay">
                    <div class="create-choice-modal glass-card">
                        ...
                        <div class="create-choice-grid">
                            ...
                            <a href="xtraP5.html" class="create-choice-btn">
                ...
                const renderPosts = (type) => {
                    ...
                    filtered.forEach(post => {
                        ...
                        if (post.format === 'image') { // Graph posts
                            ...
                            thumbnailHTML = `<img src="${post.videoUrl}" style="width: 100%; height: 100%; object-fit: cover; background: #000;">`;
                        } else if (post.format === 'p5js') { // p5.js posts
                            thumbnailHTML = `<img src="${post.videoUrl}" style="width: 100%; height: 100%; object-fit: cover; background: #000;">`;
                        ...
                        div.innerHTML = `
                            <div class="post-thumbnail" style="width:100%; height:100%; background: #111; position: relative;">
                                ${thumbnailHTML}
                                <div style="position: absolute; top: 8px; right: 8px; color: white; font-size: 1.2rem; text-shadow: 1px 1px 3px rgba(0,0,0,0.7);">${post.originalId ? '<i class="ri-flashlight-fill"></i>' : (post.format === 'image' ? '<i class="ri-bar-chart-fill"></i>' : (post.format === 'pdf' ? '<i class="ri-book-open-fill"></i>' : (post.format === 'p5js' ? '<i class="ri-palette-fill"></i>' : '<i class="ri-clapperboard-fill"></i>')))}</div>
                            </div>
        ...
        if (currentPage.includes('explore.html') || currentPage.includes('reels.html')) {
            ...
            if (exploreFeed && savedPosts.length > 0) {
                ...
                savedPosts.forEach(post => {
                    ...
                    if (post.format === 'image' && !post.pdfUrl) { // Handle static Graphs
                    ...
                    } else if (post.format === 'p5js') { // Handle p5.js sketches
                        if (currentPage.includes('reels.html') && post.source && post.source.code) {
                            const p5_iframe_content = `
                                <!DOCTYPE html><html><head>
                                <script src="https://cdnjs.cloudflare.com/ajax/libs/p5.js/1.9.0/p5.min.js"><\/script>
                                <style>body { margin: 0; overflow: hidden; background: #000; }</style>
                                </head><body><script>${post.source.code}<\/script></body></html>
                            `;
                            mediaHTML = `<iframe srcdoc="${p5_iframe_content.replace(/"/g, '&quot;')}" style="width: 100%; height: 100%; border: none;"></iframe>`;
                        } else {
                            // Fallback to thumbnail on other pages
                            mediaHTML = `<img src="${post.videoUrl}" style="width: 100%; height: 100%; object-fit: cover; background: #000;">`;
                        }
                        backgroundHTML = `<div class="reel-background" style="background: #111;"></div>`;

                    } else { // Default to Video (Handles 16:9, 9:16, etc.)
                        const fullVideoUrl = post.videoUrl.startsWith('http') ? post.videoUrl : `${getBackendUrl()}${post.videoUrl}`;
                        ...
                        remixBtn.addEventListener('click', (e) => {
                            ...
                            if (post.source && post.source.engine) {
                                ...
                                switch (post.source.engine) {
                                    ...
                                    case 'p5js': editorUrl = 'xtraP5.html'; break;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let remixOriginalId = null;

    const defaultSketch = `// Welcome to XtraP5!
...
function draw() {
  ...
  // Create a looping time value
  const t = frameCount * 0.01;

  for (let i = 0; i < 12; i++) {
    const angle = t + i * PI / 6;
    const x = cos(angle) * 100;
    const y = sin(angle) * 100;
    
    const r = map(sin(t * 2 + i), -1, 1, 50, 200);
    const g = map(cos(t + i), -1, 1, 50, 200);
    const b = map(sin(t * 0.5 + i), -1, 1, 200, 50);
    ...
    // --- Load Code: Check for Remix first, then Local Save, then Default ---
    const remixMetaRaw = localStorage.getItem('remixMeta');
    if (remixMetaRaw) {
        const meta = JSON.parse(remixMetaRaw);
        if (meta.source && meta.source.engine === 'p5js') {
            codeEditor.value = meta.source.code;
            remixOriginalId = meta.originalId;
            console.log("Loaded p5.js sketch for remix from post ID:", remixOriginalId);
        }
        localStorage.removeItem('remixMeta'); // Clear after use
    } else {
        // Load saved code or use default
        codeEditor.value = localStorage.getItem('xtraP5Code') || defaultSketch;
    }
            ...
            <body>
                <script>
                    try {
                        ...
                        document.body.innerHTML = '<pre style="color:red; padding:10px;">' + e + '</pre>';
                    }
                <\/script>
            </body>
            </html>
    ...
    codeEditor.addEventListener('input', () => {
        localStorage.setItem('xtraP5Code', codeEditor.value);
    });
    ...
    if (mobileRunBtn) {
        mobileRunBtn.addEventListener('click', updatePreview);
    }

    // Publishing Logic
    publishBtn.addEventListener('click', () => {
        const title = prompt("Enter a title for your sketch:", "My p5.js Sketch");
        if (!title) return;

        // Take a snapshot of the canvas for the thumbnail
        const previewCanvas = previewFrame.contentWindow.document.querySelector('canvas');
        const thumbnailDataUrl = previewCanvas ? previewCanvas.toDataURL('image/jpeg', 0.8) : '';

        const newPost = {
            id: Date.now(),
            title: title,
            desc: "An interactive sketch created with XtraP5.",
            videoUrl: thumbnailDataUrl, // The thumbnail
            format: 'p5js', // The new format for live rendering
            timestamp: new Date().toISOString(),
            source: {
                engine: 'p5js',
        ...
        if (confirm('Sketch published to your profile! Go to profile?')) {
            window.location.href = 'profile.html';
        }
    });
});
...
I've updated the Reels renderer and the "Create New" modal to support `p5.js`.

```diff
--- a/Users/yogendrasingh/Documents/XtraAnim/src/viewmodel/script.js
+++ b/Users/yogendrasingh/Documents/XtraAnim/src/viewmodel/script.js
    }
});